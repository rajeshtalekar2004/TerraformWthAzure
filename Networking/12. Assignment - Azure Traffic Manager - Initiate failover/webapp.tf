...
resource "azurerm_app_service_custom_hostname_binding" "secondaryapp" {
  hostname            = "${azurerm_traffic_manager_profile.app-profile.fqdn}"
  app_service_name    = azurerm_windows_web_app.secondaryapp.name
  resource_group_name = local.resource_group_name

  depends_on = [
    azurerm_traffic_manager_azure_endpoint.secondaryendpoint
  ]
}
