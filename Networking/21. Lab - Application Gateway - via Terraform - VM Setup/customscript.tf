...
resource "azurerm_storage_account" "vmstore4577687raj" {
  name                     = "vmstore4577687raj"
...
resource "azurerm_storage_container" "data" {
  name                  = "data"
  storage_account_name  = "vmstore4577687raj"
  container_access_type = "blob"
  depends_on = [
    azurerm_storage_account.vmstore4577687raj
  ]
}
...
resource "azurerm_virtual_machine_extension" "vmextension" {
  ...
  settings = <<SETTINGS
    {
        "fileUris": ["https://${azurerm_storage_account.vmstore4577687raj.name}.blob.core.windows.net/data/IIS_Config_${each.key}.ps1"],
